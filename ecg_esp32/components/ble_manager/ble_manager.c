...
#define ECG_SAMPLE_BYTES sizeof(int32_t)
#define ECG_BATCH_BYTES (ECG_BATCH_SAMPLES * ECG_SAMPLE_BYTES)
...
static uint8_t ecg_batch_buffer[ECG_BATCH_BYTES];
static uint16_t ecg_batch_len = 0;
static uint8_t notify_buffer[ECG_BATCH_BYTES];

/*
 * Xử lý BLE notification chứa payload ECG.
 * XIAO gửi batch 32 mẫu = 128 byte; nếu BLE stack chia nhỏ notification,
 * hàm này gom lại đủ 128 byte rồi mới gọi callback.
 */
static int ble_on_notify(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
    if (user_ecg_cb != NULL) {
        uint16_t len = OS_MBUF_PKTLEN(ctxt->om);

        if (len > sizeof(notify_buffer)) {
            ESP_LOGW(TAG, "Notification dài bất thường: %u byte, bỏ qua để giữ đồng bộ batch", len);
            ecg_batch_len = 0;
            return 0;
        }

        if (os_mbuf_copydata(ctxt->om, 0, len, notify_buffer) != 0) {
            ESP_LOGW(TAG, "Không copy được payload BLE, reset buffer batch");
            ecg_batch_len = 0;
            return 0;
        }

        for (uint16_t offset = 0; offset < len; ) {
            uint16_t remaining_batch = ECG_BATCH_BYTES - ecg_batch_len;
            uint16_t remaining_input = len - offset;
            uint16_t copy_len = remaining_input < remaining_batch ? remaining_input : remaining_batch;

            memcpy(ecg_batch_buffer + ecg_batch_len, notify_buffer + offset, copy_len);
            ecg_batch_len += copy_len;
            offset += copy_len;

            if (ecg_batch_len == ECG_BATCH_BYTES) {
                user_ecg_cb((int32_t *)ecg_batch_buffer, ECG_BATCH_SAMPLES);
                ecg_batch_len = 0;
            }
        }
    }
